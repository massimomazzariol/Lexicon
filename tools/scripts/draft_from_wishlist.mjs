...
function buildPrompt(entry, candidates) {
  const system = [
    ...
    '4b. IDIOMS / FIXED EXPRESSIONS. If the term is an idiom or fixed multi-word expression whose meaning is NOT the sum of its words, each language\'s `text` MUST be the equivalent MEANING (a natural equivalent expression), NEVER a literal word-for-word calque - e.g. de "Ich verstehe nur Bahnhof" -> en "I don\'t understand a thing" (NEVER "I only understand train station"), it "Non capisco nulla". Set `pos` to "chunk" for such expressions.',
